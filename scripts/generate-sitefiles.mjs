...
// 解析出 origin（如 https://holepeoplelevel.com）
...
// 生成 llms-full.txt：列出所有静态 HTML + 关卡 URLs
async function generateLlmsFull() {
  ...
  try {
    ...
    const levels = Object.keys(map).map(k => parseInt(k,10))
      .filter(Number.isFinite).sort((a,b)=>a-b);
    levelUrls = levels.map(n => `${BASE_URL}/level.html?n=${n}`);
  } catch {}
...
// 生成 llms.txt：给 LLM 的入口与规则（指向 llms-full 与 sitemap）
...
async function main() {
  ...
  console.log(`👉 现在你可以访问：${BASE_URL}/sitemap.xml`);
  console.log(`👉 现在你可以访问：${BASE_URL}/llms-full.txt`);
  console.log(`👉 现在你可以访问：${BASE_URL}/llms.txt`);
